...
use sol_rpc_router::config::Backend;
use sol_rpc_router::health::{HealthState, BackendHealthStatus};
...
fn create_test_state() -> AppState {
    ...
    let label_to_url = backends.iter().map(|b| (b.label.clone(), b.url.clone())).collect();
...
fn test_select_backend_method_override() {
    let mut state = create_test_state();
    state.method_routes.insert("eth_call".to_string(), "secondary".to_string());
